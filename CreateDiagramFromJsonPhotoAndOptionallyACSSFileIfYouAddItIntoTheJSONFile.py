...
def turnJsonListWithTwoDictionariesCalledLabelAndTagWith2EntriesIntoAnActuallyUsableFormatForPythonAkaAListOfTouples(dict):
    tuplist = []
    for entry in dict:
...
def doTheSameThingAsTheFunctionDescribedAboveHoweverWithTheDifferenceThatEachListIsActuallyAListOfListsOfDictionaries(list):
    tuplist = []
    for listinlistoftuplists in list:
        tuplist.append(turnJsonListWithTwoDictionariesCalledLabelAndTagWith2EntriesIntoAnActuallyUsableFormatForPythonAkaAListOfTouples(listinlistoftuplists))
    return tuplist
...
for legend in data["legendList"]: #Iterate over the list of legends
    ...
    temporaryLegendStorage = turnJsonListWithTwoDictionariesCalledLabelAndTagWith2EntriesIntoAnActuallyUsableFormatForPythonAkaAListOfTouples(data[legend])
...
for Label in graphicPinLabelGroups:
    labels = doTheSameThingAsTheFunctionDescribedAboveHoweverWithTheDifferenceThatEachListIsActuallyAListOfListsOfDictionaries(data[Label[6]])
    print(Label[0:2])
    graphic.add(
        PinLabelGroup(
            x = Label[0],
...
imgFilepath = dataFilepath.replace("/MyJson.json", "/diagram.svg")
